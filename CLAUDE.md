...
- `test/verify-browser-3d.mjs`：立體渲染模式的真瀏覽器驗收（另開一支，不跟上面那支混在一起）。
  ...
  ⚠ **整支測試的所有情境都開 `serviceWorkers: "block"`**——這支在意的是 3D 渲染本身，
  不是 PWA/離線行為（那是 `verify-browser.mjs` 的工作），裝著 SW 反而引來兩顆跟 3D 無關的坑：
  ① 模擬「render3d.js 載入失敗」時，攔 `sw.js` 的請求（例如回 404）**攔不住** SW 繼續掌控頁面：
    SW 一旦裝上就會用自己的 fetch 事件處理常式，把已預先快取的 `render3d.js` 直接端出來，
    那條路徑不經過 `page.route()` 的攔截。
  ② 2026-09-25 對著**剛部署完、這台瀏覽器沒造訪過**的 netlify.app 首次實測：`sw.js` 的
    `clients.claim()` 觸發的 `window.location.reload()` 會在頁面「看起來已經穩定」之後
    幾百毫秒才發生（時間點跟下載/安裝 SW 與其預先快取清單要花多久綁在一起，本來就跟
    網路狀況一起飄），剛好攔腰打斷「勾選開關→按開始→點畫布」這段互動，畫面被換回最初的
    設定頁。症狀完全不像「reload 打斷了」——反而很像轉橫向後版面跑掉、或設定沒生效，
    連續好幾支獨立的診斷腳本才追出真因。固定多睡幾秒能降低撞見的機率但躲不掉，
    乾脆整支測試都不裝 SW，從根上拿掉這個變因。
  ★ 這也是為什麼「轉橫向後 3D 場景還在」還要**另外**斷言 `!gameScreen.hidden`：
    `window.__render3dDebug()` 讀的是 Three.js 場景本身，就算頁面已經跳回設定畫面，
    它也可能還吐得出「看起來正常」的舊資料（場景物件沒被清掉）——單獨那項測不出
    「其實已經離開遊戲畫面」。
...
- **鏡頭角度是「固定俯角＋依尺寸重算距離」，不是每種長寬比各調一次**：用球場對角線的一半當半徑，
  ...
  這條原本寫「是刻意的取捨，不是 bug」——**2026-09-26 第三輪被使用者的截圖推翻**，見下一條。
...
- **危險線是 3D 場景自己的物件（2026-09-26 補），不是畫在 2D 疊層上**：初版把危險線留在
  2D 疊層裡（跟道具/子彈/狀態列一樣），理由是「它是平的一條線，2D canvas 的 dashed line
  API 沒有對應的 3D 概念，抽出來似乎划不來」——**這個判斷後來被使用者實機回報推翻**：
  「立體模式下，線跑到板子下面去了」。真因是 2D 疊層走的是**固定螢幕百分比**的平面映射，
  板子是**透視投影**的 3D 物件，同一個 `canvasY` 在兩套映射下對不上——板子（近景，透視放大）
  反而畫到「線」（平面，不受透視影響）的上面。修法是把危險線也做成 `render3d.js` 裡的
  `dangerLineMesh`，跟板子吃同一顆鏡頭、同一套透視，兩者的相對位置在任何鏡頭角度下都保證一致。
  ⇒ **教訓**：3D 場景裡任何會跟「3D 物件」比相對位置的東西（哪怕本身邏輯上是平面/2D 的），
  只要要跟 3D 物件對齊，就得進 3D 場景本身，不能留在扁平疊層裡用「同一個座標」蒙混過去。
- **寶物道具也是 3D 場景自己的物件（2026-09-26 再補一次）**：教訓沒有一次記取——
  上面那條剛修完危險線，寶物道具（`drawPowerups()`）當時也留在 2D 疊層，理由是「它會動、
  形狀簡單，等使用者真的抱怨再說」。**使用者馬上就抱怨了**：「吃不到寶物」。真因一模一樣
  （道具是平面映射、板子是透視投影，玩家對著畫面上的道具去接，板子的實際位置卻不在那裡；
  `updatePowerups()` 的碰撞判定本身完全沒壞，是畫面對不準）。修法：道具改成 `render3d.js`
  的 `THREE.Sprite`（永遠面向鏡頭的內建 billboard，不用像磚塊那樣手動算貼合鏡頭的旋轉），
  貼圖用 Canvas 現畫一次、以「顏色＋字樣」為 key 快取，同一種道具全場共用一張圖。
  ⇒ 這條教訓現在才算真的記住：**這個遊戲只要是「玩家需要對準板子接住的東西」，
  一律要進 3D 場景，不會再有例外了**（目前只剩子彈/護盾條/浮動分數/狀態列這些純顯示、
  不需要玩家瞄準對位的東西留在 2D 疊層）。
- **直向手機的「拇指區」已經拿掉（2026-09-26 第二輪）**：原本板子離底邊留畫布高度 8.5%
  的空間，讓單手直握時大拇指不會擋住板子；使用者兩輪都回報「還是太遠／請移到底」，
  在被明確告知「移到底之後手指可能擋住板子」這個取捨後仍然要求貼底，`getPaddleBottomGap()`
  改成跟橫向共用同一個固定像素常數 `PADDLE_BOTTOM_GAP`（28px），不再依畫布高度算百分比。
  `test/patterns.mjs` 原本「拇指區至少留 6%」的安全網也跟著拿掉（不是漏寫，是使用者的
  決定已經蓋過那條舊的預設值）。真的要復原拇指區，請先跟使用者確認。
- **鏡頭改成不對稱視錐（off-axis frustum），把「刻意的取捨」修成真的貼底（2026-09-26 第三輪）**：
  上一輪把板子的**邏輯座標**移到最底之後，使用者又截圖問「線與擋板有到最底嗎」——量出來
  `paddle.y` 確實只剩 28px 到畫布底邊，但**鏡頭投影到螢幕上**卻還有一大截空白，真因就是
  上面那條寫的「刻意取捨」：直向手機球場又窄又長，鏡頭距離被「左右不能裁到磚牆」那頭撐得
  比「縱深方向塞得進去」需要的還遠，而 `camera.updateProjectionMatrix()` 用的是上下左右**對稱**
  的視角，多出來的視角配額因為對稱硬要平分掉，整段留在畫面下方（板子那一端）。
  修法：`resize()` 改成手動算 `camera.projectionMatrix.makePerspective(left, right, top, bottom, near, far)`——
  左右仍用原本對稱公式（那頭本來就抓得緊，沒有裁切風險，不要動），只把上/下拆開，
  分別對準板子那一端（近景）與磚牆/上牆那一端（遠景）實際需要的角度，鏡頭距離完全不變。
  ⚠ 記得設定 `camera.projectionMatrixInverse` 也要跟著手動算的矩陣同步更新，
  且之後不能再呼叫 `camera.updateProjectionMatrix()`（那會用回對稱公式蓋掉這個修正）。
  驗收新增 `window.__render3dDebug().paddleScreenFrac`（板子投影到螢幕後離最下緣的比例，
  0=貼底、1=頂到最上緣），比截圖比對更精準也能寫進自動化測試；改用不對稱視錐後直向手機
  量到 ≈0.88（落在畫面最下面一段）。已用 `git stash`／手動換回舊版 `render3d.js` 驗證過
  這條測試對舊版真的會抓到（舊版沒有這個欄位，斷言直接失敗，不是擺著好看的斷言）。
